...
- Terminalのプロンプトの改良（導入まで）
- パッケージマネージャの導入
- Gitの設定（GitHub連携を含む）

今回はいかなる開発環境下でも必須となるであろう上記3点を扱う。
...
2. インストールできているか確認する。
    ...
    上記`output example`のような出力が得られていれば完了。
    (`手順3`と`手順4`は飛ばして良い。)
    &nbsp;

3. インストールできていなければパスを通す。
...
1. さきほど導入したHomebrewを使ってインストールする。
...
5. 接続テストを行う。
    ...
    私はすでに接続を確認しているので、上記`output example`を受け取りますがはじめて接続した場合にはもう少し長い出力を受け取ります。その際には、`Enter`や`yes`を適宜入力して先に進めてください。最後に上記`output example`と同じような文言が出力されます。
